...
import 'package:rick_and_morty_app/core/extentsions/list.dart';
...
void main(args) async {
  ...
  final translated = await processEntities(
    languages,
    content.map((e) => LocaleCsvEntity.fromList(e, languages)).toList(),
...
Future<List<LocaleCsvEntity>> translateContent(
  ...
  List<LocaleCsvEntity> needTranslate = list.where((entity) {
    if (ignoreWith.any((element) => entity.key.contains(element))) {
      return false;
    }
